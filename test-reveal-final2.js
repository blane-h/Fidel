...
setTimeout(() => {
  ...
  // Step 2: Click first consonant → reveals its vowel
  haBtn?.click();
  let revealedAfterHa = doc.querySelectorAll('.revealed-correct').length;
  console.log('Step 2: After clicking ሀ, revealed vowels:', revealedAfterHa);
  
  // Step 3: Click next consonant (ለ) → should highlight AND reveal its vowel
  laBtn?.click();
  ...
  let revealedAfterLa = doc.querySelectorAll('.revealed-correct').length;
  console.log('Step 3: After clicking ለ, revealed vowels:', revealedAfterLa, 'next consonant highlighted:', laHighlighted);
  
  // Check for any setTimeout in reveal functions
  const revealFunctions = appJs.match(/function reveal.*?\{[^}]*\}/gs) || [];
  const hasTimer = revealFunctions.some(f => f.includes('setTimeout'));
  
  console.log('\n---');
  console.log('Has setTimeout in reveal functions:', hasTimer ? 'YES ❌' : 'NO ✓');
  console.log('First consonant auto-highlighted:', haHighlighted ? 'YES ✓' : 'NO ❌');
  console.log('Clicking next consonant triggers vowel reveal:', laHighlighted && revealedAfterLa >= 1 ? 'YES ✓' : 'NO ❌');
  
  const passed = haHighlighted && laHighlighted && revealedAfterLa >= 1 && !hasTimer;
