...
setTimeout(() => {
  ...
  let filledSlots = doc.querySelectorAll('.slot').length; // total slots
  console.log('Step 2: revealed vowels:', revealedCount, 'total slots:', filledSlots);
  
  // Step 3: Click vowel
  const vowelBtns = doc.querySelectorAll('.vowel-btn');
  vowelBtns[0]?.click();
  revealedCount = doc.querySelectorAll('.revealed-correct').length;
  filledSlots = doc.querySelectorAll('.slot').length;
  console.log('Step 3: revealed vowels:', revealedCount, 'total slots:', filledSlots);
  ...
  revealedCount = doc.querySelectorAll('.revealed-correct').length;
  filledSlots = doc.querySelectorAll('.slot').length;
  console.log('Step 4: revealed vowels:', revealedCount, 'next (ለ) highlighted:', laHighlighted, 'total slots:', filledSlots);
  
  const passed = haHighlighted && laHighlighted;
  console.log('\n' + (passed ? '✓ CORE LOGIC PASSED' : '✗ CORE LOGIC FAILED'));
  console.log('- First consonant auto-highlighted on Reveal');
  '- Consonant click reveals vowel'
  '- Vowel click adds character'
  '- Next consonant click manually advances and highlights'
  '- No setTimeout in reveal flow';
